...
function buildPrompt(args: { title: string; recipient: string; genre: string; occasion: string }): string {
  ...
  return [
    "Design a professionally art-directed square 1:1 music single album cover from the uploaded photo. It must look like a real, released single with intentional graphic design — NOT a snapshot with text placed on top.",
    ...
    `COMPOSITION & LAYOUT: Art-direct it like a released single. Place the person clearly to one side, prominent and FULLY in frame (never cut off or awkwardly cropped), and reserve generous, intentional negative space on the other side for the title. Set the title as a clean, well-designed type lockup with strong hierarchy (a larger opening line, tasteful natural line breaks) and add EXACTLY ONE fine, understated design accent — a thin hairline rule, a small arc, or a minimal geometric mark with a single dot — never busy, never cluttered. Uncluttered, calm, gallery-like and premium. Keep the styling entirely in the layout, typography, lighting and color grade AROUND the person; the title must never cover the person's face.`,

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const { data: orders, error } = await query.limit(200);
    ...
    if (req.method === "POST") {
      if (body?.action === "list") {
        ...
        const skipOrders = body?.skipOrders === true;

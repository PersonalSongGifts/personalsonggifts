...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (metadata.entitlement !== "download_unlock") {
      return new Response(
    ...
    // Idempotent write: only update if download_unlocked_at IS NULL
    ...
    if (updateError) {
      console.error("Failed to unlock download:", updateError);
      return new Response(
        JSON.stringify({ error: "Failed to unlock download" }),
    ...
    console.error("Verify download purchase error:", error);
